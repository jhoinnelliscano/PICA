...
namespace eCommerce.Products.Availability.Infraestructure.Contexts.DbProduct
{
    public partial class DbProductContext : DbContext
    {
        ...
        public async Task<bool> SpUpdateProductStockAsync(long productId, int units)
        {
            var result = await this.Database.ExecuteSqlRawAsync($"EXEC {_spUpdateProductStock} @productId , @units",

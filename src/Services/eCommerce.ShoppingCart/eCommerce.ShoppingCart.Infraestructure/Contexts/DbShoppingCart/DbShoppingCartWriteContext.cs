...
namespace eCommerce.ShoppingCart.Infraestructure.Contexts
{
    public partial class DbShoppingCartWriteContext : DbContext
    {
        ...
        public async Task<bool> SpUpdateShoppingCartStatusAsync(long porductId, bool status)
        {
            var result = await this.Database.ExecuteSqlRawAsync($"EXEC {_spUpdateShoppingCart} @status, @productId",
                new SqlParameter("@status", status),
                new SqlParameter("@productId", porductId));
